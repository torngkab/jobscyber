...
<?
	...
	//เลือกข้อมูลมาแสดง
	$strSQL = "select * from customer_check_see_status INNER JOIN employee_resume_th ON customer_check_see_status.idresume = employee_resume_th.idresume WHERE customer_check_see_status.idcompany='$_SESSION[idcompanyvalue]'";	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
...
<?
	while($row1=mysql_fetch_array($objQuery)) {   //  start  while 1
		echo "<tr>";
		$d = substr($row1[date], -2);
		$m = substr($row1[date], -4, 2);
